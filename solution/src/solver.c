...
void solve(Solver* solver)
{
    ...
    while ((res >= epssq) && (it < itermax)) {
        ...
        if (isBoundary(solver, BOTTOM)) { // set bottom bc
            for (int i = 1; i < imaxLocal + 1; i++) {
                P(i, 0) = P(i, 1);
            }
        }

        if (isBoundary(solver, TOP)) { // set top bc
            for (int i = 1; i < imaxLocal + 1; i++) {
                P(i, jmaxLocal + 1) = P(i, jmaxLocal);
            }
        }

        if (isBoundary(solver, LEFT)) { // set left bc
            for (int j = 1; j < jmaxLocal + 1; j++) {
                P(0, j) = P(1, j);
            }
        }

        if (isBoundary(solver, RIGHT)) { // set right bc
            for (int j = 1; j < jmaxLocal + 1; j++) {
