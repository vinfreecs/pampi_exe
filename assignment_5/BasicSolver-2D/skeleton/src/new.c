...
// {
//     /* Use solver->comm, NOT MPI_COMM_WORLD */
...
// {
//     /* FIXED: Check against DIMENSIONS, not SIZE */
//     int dim_x = solver->dims[IDIM];
//     int dim_y = solver->dims[JDIM];
...
//         break;
//     case RIGHT:
//         return solver->coords[IDIM] == dim_x - 1; // Fixed
//         break;
//     case BOTTOM:
//         return solver->coords[JDIM] == 0;
//         break;
//     case TOP:
//         return solver->coords[JDIM] == dim_y - 1; // Fixed
//         break;
//     }
//     return 1;
// }
...
//     /* FIXED: Allocate array to keep datatypes alive during Waitall */
//     MPI_Datatype *domainTypes = NULL;

//     if (solver->rank == 0)
//     {
//         domainTypes = (MPI_Datatype *)malloc(solver->size * sizeof(MPI_Datatype));
...
//     /* Clean up types only after Waitall is done */
...
//     double *p = solver->p;
//     /* FIXED: Added rhs declaration for the RHS macro */
//     double *rhs = solver->rhs;
...
//         /* FIXED: Use LOCAL sizes for boundary loops */
...
// {
//     /* FIXED: Use local size for iteration */
//     int size = (solver->imaxLocal + 2) * (solver->jmaxLocal + 2);
//     double *p = solver->p;
//     double avgP = 0.0;
...
//     avgP /= (solver->imax + 2) * (solver->jmax + 2); // Unsure if this normalization logic is fully intended by user, but following existing pattern
...
//     {
//         /* FIXED: Use <= to include the last element */
...
//         for (int j = 1; j < jmaxLocal + 1; j++)
//         {
//             /* FIXED: Calculate global J to find correct physical Y */
//             int globalJ = (solver->coords[JDIM] * solver->jmaxLocal) + j; // Assuming uniform distribution
